...
class UnixSocketBridge: ObservableObject {
    ...
    private var frameBuffer: MTLSharedEvent?
    private var socketPath: String

    private let mousePosition = CGPoint.zero
    ...
    func startServer() {
        inputQueue.async { [weak self] in
            ...
            do {
                ...
                let listenEndpoint = NWEndpoint.unix(path: url.path)
                self.listener = try NWListener(using: params, on: listenEndpoint)

                self.listener?.stateUpdateHandler = { state in
                    switch state {
                    case .ready:
                        print("[Socket] Server ready on \(self.socketPath)")
                ...
                self.listener?.start(queue: self.inputQueue)
                print("[Socket] Input server started")
            } catch {
                print("[Socket] Failed to start server: \(error)")
            }
        }
    }
    ...
    private func handleNewConnection(_ newConnection: NWConnection) {
        ...
        newConnection.start(queue: inputQueue)
    }
    ...
    func sendKeyPress(_ key: String, pressed: Bool = true) {
        let event = InputEvent(
            ...
            timestamp: Date().timeIntervalSince1970,
            data: InputData(key: key, pressed: pressed)
        )
        sendEvent(event)
    }
