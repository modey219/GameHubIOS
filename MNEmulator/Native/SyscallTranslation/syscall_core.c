...
/* Redirect host libc calls to raw syscalls. LiveContainer's interposer
   corrupts/hangs the legacy path functions (open/stat/access/fstat), and
   this file runs them for every x86 syscall wine issues. box64_raw_* go
   straight to the kernel so no interposer can touch them. */
